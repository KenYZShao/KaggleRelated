...
//#include "Camera.cpp"//Shao added, not sure should it be removed.
...
//Global variables for 1st/3rd person's view
glm::vec3 startPosition2 = glm::vec3(0.0f, 0.0f, -10.0f);
glm::vec3 position;
glm::vec3 front;
glm::vec3 up;
glm::vec3 right;
glm::vec3 worldUp;
glm::vec3 rotation;//Shao edited referencing from a Youtube tutorial
/*
//glm::quat aroundY;
// Yaw
...
*/
GLfloat yaw;//Shao explained, turning the neck
GLfloat pitch;//Shao explained,looking at the sky and looking at the ground

GLfloat moveSpeed;
GLfloat turnSpeed;


// Referenced from https://stackoverflow.com/questions/28776788/quaternion-reaching-gimbal-lock
// vertex array
...
//Quaternion for Gimbal Lock

//Global positions up to here

//Below code is referenced from Zhang Shijun for 1st/3rd person view
...
/*
void MyCamera00(glm::vec3 startPosition, glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed) {
	glm::vec3 position = startPosition;//Original from Udemy, here could be changed for 1st or 3rd person view.
	glm::vec3 worldUp = startUp;
	GLfloat yaw = startYaw;
	GLfloat pitch = startPitch;
	glm::vec3 front = glm::vec3(0.0f, 0.0f, -1.0f);

	GLfloat moveSpeed = startMoveSpeed;
	GLfloat turnSpeed = startTurnSpeed;

	void update();
}

void MyCamera1st(glm::vec3 startPosition, glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed) {
	glm::vec3 position = startPosition;//Original from Udemy, here could be changed for 1st or 3rd person view.
	glm::vec3 worldUp = startUp;
	GLfloat yaw = startYaw;
	GLfloat pitch = startPitch;
...
void MyCamera2nd(glm::vec3 startPosition, glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed) {
	glm::vec3 position = startPosition + glm::vec3(0.0f, 0.0f, -10.0f);//Original from Udemy, here could be changed for 1st or 3rd person view.
	glm::vec3 worldUp = startUp;
	GLfloat yaw = startYaw;
	GLfloat pitch = startPitch;
	glm::vec3 front = glm::vec3(0.0f, 0.0f, -1.0f);
...
*/
Camera MyCamera(glm::vec3(0, 0.0f, 0.0f), glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed);
Camera MyCamera1st(glm::vec3(0, 0.0f, 0.0f), glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed);
Camera MyCamera3rd(glm::vec3(0, 0.0f, -10.0f), glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed);
//Camera.MyCamera1::position = startPosition + glm::vec3(0.0f, 0.0f, -10.0f);
//Camera MyCamera2nd(glm::vec3 startPosition, glm::vec3 startUp, GLfloat startYaw, GLfloat startPitch, GLfloat startMoveSpeed, GLfloat startTurnSpeed);
#pragma endregion
//Up to above code is referenced from Zhang Shijun for 1st/3rd person view
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Shao edited here for implementing 1st/3rd person's view, got help from Shijun Zhang
		if (mainWindow.getsKeys()[GLFW_KEY_1]) {
			Mycamera = MyCamera1st;
		}
		else if (mainWindow.getsKeys()[GLFW_KEY_3]) {
			MyCamera = MyCamera3rd;
		}
		if (mainWindow.getsKeys()[GLFW_KEY_UP])
		{
			MyCamera.speedX = 10.0f;
			rotation.x += MyCamera.speedX;
		}
		...
		if (mainWindow.getsKeys()[GLFW_KEY_LEFT]) {
			MyCamera.speedY = 10.0f;
			rotation.y += MyCamera.speedY;//+=
		}
		...
		if (mainWindow.getsKeys()[GLFW_KEY_U]) {
			MyCamera.speedZ = 10.0f;
			rotation.z += MyCamera.speedZ;//+=
		}
